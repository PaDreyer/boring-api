...
function generatedFile(
    ...
    const lines: string[] = [
        ...
        'import type { Context as BoringContext } from "boring-api";',
...
function generatedContracts(outputFile: string, contracts: ContractSource[]): string {
    const lines = [
        "// Generated by boring-api. Do not edit.",
        'import type { z } from "zod";',
        "",
...
export function generateTypes(projectRoot: string, apiDirectory: string): TypegenResult {
    ...
    if (tree.contracts.length) {
        ...
        writeFileSync(contractsFile, generatedContracts(contractsFile, tree.contracts));
