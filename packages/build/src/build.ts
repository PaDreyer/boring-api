...
export function buildProject(project: AnalyzedProject): { diagnostics: readonly ts.Diagnostic[]; output: string } {
    ...
    for (const directory of [project.generatedRoot, project.apiDirectory, join(dirname(project.apiDirectory), "modules"), join(dirname(project.apiDirectory), "infra"), join(dirname(project.apiDirectory), "web"), join(dirname(project.apiDirectory), "executions"), join(dirname(project.apiDirectory), "jobs")]) {
    ...
    for (const reserved of [startup, worker, marker]) {
