...
- Browser code uses `@boringapi/core/client` and generated `ApiRoutes` from the API's `$client` through `import type`. Generated client contracts must remain standalone without server imports. Server pages reuse injected facades and never import database adapters or SDKs.
