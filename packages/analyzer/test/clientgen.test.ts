...
import { ApiError, createClient } from "@boringapi/core/client";
    ...
    try {
        for (const [file, content] of Object.entries({
            ...
            "tsconfig.json": JSON.stringify({ compilerOptions: {
                ...
                baseUrl: ".", paths: { "@boringapi/core": [require.resolve("@boringapi/core").replace(/\.js$/, ".d.ts")],
                    "@boringapi/core/client": [require.resolve("@boringapi/core/client").replace(/\.js$/, ".d.ts")], "$client": [".boring/types/api/$client.d.ts"], zod: [join(repository, "node_modules/zod")] },
            } }),
...
it("resolves the published browser subpath with TypeScript 4.9 and no server runtime", () => {
    consumer({}, root => {
        ...
        cpSync(require.resolve("@boringapi/core/package.json"), join(packageRoot, "package.json"));
        mkdirSync(join(packageRoot, "dist"));
        for (const extension of ["js", "d.ts"]) {
            cpSync(require.resolve("@boringapi/core/client").replace(/\.js$/, `.${extension}`), join(packageRoot, `dist/client.${extension}`));
        }
