...
import type { OperationalRecord } from "@boringapi/core";
import { createOperations } from "../infra/operations";

it("serializes operational writes behind one shared drain without accumulating listeners", async () => {
    const stream = new EventEmitter() as EventEmitter & { write(line: string): boolean; lines: string[] };
    stream.lines = [];
    let blocked = true;
    stream.write = line => { stream.lines.push(line); return !blocked; };
    const adapter = createOperations(stream);
    const record: OperationalRecord = { kind: "log", timestamp: 1, level: "info", event: "test", message: "test", attributes: {} };
    const writes = Array.from({ length: 100 }, () => Promise.resolve(adapter.emit(record)));
    ...
    await adapter.flush?.();
    assert.equal(stream.lines.length, 100);
...
console.error("setup pending");await new Promise(resolve=>setTimeout(resolve,100));return app;};require("./server.ts");`;
    const child = spawn(process.execPath, ["-r", "ts-node/register", "-r", "./test/register.ts", "-e", script], {
        ...
        env: { ...process.env, DATABASE_URL: "postgres://unused/boring", PORT: "0" },
    });
    ...
    assert.doesNotMatch(output, /Fullstack example listening/);
});
...
http:(_request,_response,next)=>next(),health:()=>({status:"up"}),readiness:async()=>({status:"ready",checks:[]}),metrics:()=>[],
...
get closed(){return closed;}};};require("./server.ts");`;
    ...
    assert.ok(sent, output); assert.equal(child.signalCode, null, output); assert.equal(child.exitCode, 0, output);
    assert.equal((output.match(/cleanup finished/g) ?? []).length, 1, output);
    assert.doesNotMatch(output, /Fullstack example listening|Application shut down while listening/);
...
get closed(){return closed;}};};require("./server.ts");`;
    ...
    const failedForce = setTimeout(() => failed.kill("SIGKILL"), 15000);
...
if(${JSON.stringify(failCleanup)}){const error=new Error("signal cleanup failure");reject(error);throw error;}settle();},get closed(){return closed;}};};require("./server.ts");`;
        ...
        if (failCleanup) { assert.match(output, /Server runtime and cleanup failed/); assert.match(output, /signal cleanup failure/); }
    }
});
