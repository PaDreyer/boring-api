...
import { ApiSources, ContractSource, RouteSource, scanApi } from "@boringapi/core/conventions";
import { inside, conventionPaths } from "@boringapi/compiler";
...
function generatedFile(outputFile: string, directory: string, routes: RouteSource[], tree: ApiSources): string {
    ...
    const config = tree.config ? moduleType(tree.config) : undefined;
    const setup = tree.setup ? moduleType(tree.setup) : undefined;
    const auth = tree.auth ? moduleType(tree.auth) : undefined;
    const definitions: string[] = [
        `export type JobInputs = { ${tree.jobs.map(job => `${JSON.stringify(job.name)}: z.input<${moduleType(job.file)}["payload"]>`).join("; ")} };`,
        config ? `export type Config = z.output<${config}["schema"]>;` : "export type Config = Readonly<Record<string, never>>;",
        ...
        "export type Session = [Exclude<RawSession, void | null>] extends [never] ? ExecutionIdentity : Exclude<RawSession, void | null>;",
    ...
    if (hooks.has("+config")) exports.push("export type ConfigEnvironment = Readonly<Record<string, string | undefined>>;");
    if (hooks.has("+setup")) exports.push(
        "export type SetupContext = BoringSetupContext<Config, JobInputs>;",
    ...
    return [
        ...
        'import type { Context as BoringContext, SetupContext as BoringSetupContext, ExecutionContext, ExecutionIdentity } from "@boringapi/core";',
        ...
        "",
        ...imports, "", ...definitions,
        'type RequestContext<S, L> = Omit<BoringContext, "session" | "services" | "locals" | "execution"> & {',
        "    readonly session: S;",
        "    readonly execution: ExecutionContext<S extends ExecutionIdentity ? S : undefined>;",
...
function generatedContracts(outputFile: string, contracts: ContractSource[], auth: string | undefined): string {
    const lines = [
        "// Generated by Boring API. Do not edit.",
        'import type { z } from "zod";',
        'import type { ExecutionIdentity } from "@boringapi/core";',
        "",
        auth
        ...
        "type ConfigContract = { schema: z.ZodTypeAny; load: (env: Readonly<Record<string, string | undefined>>) => unknown };",
        "type AuthContract =",
        "    | { authenticate: (...args: any[]) => ExecutionIdentity | void | Promise<ExecutionIdentity | void>; authorize?: AnyFunction }",
        "    | { authenticate?: (...args: any[]) => ExecutionIdentity | void | Promise<ExecutionIdentity | void>; authorize: AnyFunction };",
        ...
        "type AssertConfig<T extends ConfigContract> = T;",
...
export function generateTypes(projectRoot: string, apiDirectory: string): TypegenResult {
    ...
    // Validate all deletion/write destinations before modifying generated files.
    const kinds = ["job", "schedule", "event", "command"] as const;
    const generatedEntries = kinds.map(kind => {
        const sourceRoot = join(dirname(api), `${kind}s`);
        childPath(root, sourceRoot, "Entry directory");
        if (existsSync(sourceRoot)) childPath(root, realpathSync(sourceRoot), "Entry directory");
        const target = join(generatedRoot, relative(root, sourceRoot));
        rejectSymbolicLinkPath(root, target);
        return target;
    });
    rejectSymbolicLinkPath(root, generatedApiRoot);
    rmSync(generatedApiRoot, { recursive: true, force: true });
    for (const target of generatedEntries) rmSync(target, { recursive: true, force: true });
    const files: string[] = [];
    for (const kind of kinds) for (const entry of tree[`${kind}s`]) {
        const title = kind[0].toUpperCase() + kind.slice(1);
        const output = join(generatedRoot, relative(root, dirname(entry.file)), "$types.d.ts");
        mkdirSync(dirname(output), { recursive: true });
        const setupType = tree.setup ? `Awaited<ReturnType<typeof import(${JSON.stringify(moduleSpecifier(output, tree.setup))}).setup>>` : "{}";
        writeFileSync(output, [
            "// Generated by Boring API. Do not edit.",
            `import type { ${title}Context as Context } from "@boringapi/core";`,
            'import type { z } from "zod";',
            `type Declaration = typeof import(${JSON.stringify(moduleSpecifier(output, entry.file))});`,
            `export type ${title}Context = Context<z.output<Declaration["${kind === "command" ? "input" : "payload"}"]>, ${setupType}>;`,
            kind === "command" ? 'type Result = z.input<Declaration["output"]>;' : "type Result = void;",
            `export type ${title}Handler = (ctx: ${title}Context) => Result | Promise<Result>;`, "",
        ].join("\n"));
        files.push(output);
    }
    ...
    if (tree.contracts.length || kinds.some(kind => tree[`${kind}s`].length)) {
        ...
        writeFileSync(contractsFile, generatedContracts(contractsFile, tree.contracts, tree.auth) +
            kinds.map(kind => {
                const title = kind[0].toUpperCase() + kind.slice(1);
                return `type Assert${title}<T extends import("@boringapi/core").${title}Declaration> = T;\n` +
                    tree[`${kind}s`].map((entry, index) => {
                        const declaration = `typeof import(${JSON.stringify(moduleSpecifier(contractsFile, entry.file))})`;
                        const context = join(generatedRoot, relative(root, dirname(entry.file)), "$types.d.ts");
                        const generated = `import(${JSON.stringify(moduleSpecifier(contractsFile, context).replace(/\.d$/, ""))})`;
                        return `type ${title}${index} = Assert${title}<${declaration}>;\n` +
                            `type Check${title}${index}<F extends ${generated}.${title}Handler> = F;\ntype Handler${title}${index} = Check${title}${index}<${declaration}["handler"]>;\n` +
                            (kind === "schedule" ? `type Input${index}<I extends import("zod").input<${declaration}["payload"]>> = I;\ntype ScheduleInput${index} = Input${index}<${declaration}["input"]>;` : "");
                    }).join("\n");
            }).join("\n") + "\n");
