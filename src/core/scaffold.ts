...
import { formatArchitectureDiagnostics } from "./architecture";
import { inside, moduleLiteral, resolveImport } from "./compiler";
import { endpointSegment, HTTP_METHODS, RouteSource, SourceScope } from "./conventions";
import { inspectProject } from "./inspect";
import { analyzeProject, AnalyzedProject, formatHost } from "./project";
...
export function initializeProject(directory: string, apiDirectory = "api"): ScaffoldResult {
    ...
    dependencies["@boringapi/core"] ??= devDependencies["@boringapi/core"] ?? `^${own.version}`;
    dependencies.zod ??= devDependencies.zod ?? own.peerDependencies.zod;
    const changes: Change[] = [{ file: manifestFile, before, content: JSON.stringify({ ...manifest, scripts, dependencies }, null, 2) + "\n" },
