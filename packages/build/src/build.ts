...
export function buildProject(project: AnalyzedProject): { diagnostics: readonly ts.Diagnostic[]; output: string } {
    ...
    const triggers = ["scheduler", "schedule-worker", "consumer", "command"].map(name => join(outDir, `boring-${name}.cjs`));
...
const { join } = require("node:path");
const { BoringApi } = require("@boringapi/core");
new BoringApi().createApp(join(__dirname, ${JSON.stringify(apiDirectory)})).then(async application => {
    for (const signal of ["SIGINT", "SIGTERM"]) process.once(signal, () => {
        void application.close().catch(error => { console.error(error); process.exitCode = 1; });
    });
...
`);
    for (const file of triggers) {
        const mode = file === triggers[0] ? "scheduler" : file === triggers[1] ? "schedule" : file === triggers[2] ? "event" : "command";
...
const stop = () => {
    stopping = true;
    cancellation.abort();
    // The awaited cleanup below owns error reporting, including signal failures.
...
(async () => {
    ...
    try {
        ...
        try { if (application) { await application.close(); await application.closed; } }
    ...
    if (failed) {
        const description = commandFailure(failure);
...
})().catch(error => { const failure = commandFailure(error); console.error(JSON.stringify({error:failure.error})); process.exitCode = failure.exitCode; });
`);
    }
