...
export function generateTypes(projectRoot: string, apiDirectory: string): TypegenResult {
    ...
    // Job declarations mirror sibling source paths; validate every deletion/write first.
    const jobsRoot = join(dirname(api), "jobs");
    childPath(root, jobsRoot, "Jobs directory");
    if (existsSync(jobsRoot)) childPath(root, realpathSync(jobsRoot), "Jobs directory");
    const generatedJobs = join(generatedRoot, relative(root, jobsRoot));
    rejectSymbolicLinkPath(root, generatedJobs);

    rejectSymbolicLinkPath(root, generatedApiRoot);
    rmSync(generatedApiRoot, { recursive: true, force: true });
    rmSync(generatedJobs, { recursive: true, force: true });
    const files: string[] = [];
    for (const job of tree.jobs) {
        const output = join(generatedRoot, relative(root, dirname(job.file)), "$types.d.ts");
        ...
        writeFileSync(output, [
            "// Generated by Boring API. Do not edit.",
            'import type { JobContext as Context } from "@boringapi/core";',
            'import type { z } from "zod";',
            `type Declaration = typeof import(${JSON.stringify(moduleSpecifier(output, job.file))});`,
            `export type JobContext = Context<z.output<Declaration["payload"]>, ${setupType}>;`,
    ...
    if (tree.contracts.length || tree.jobs.length) {
        ...
        writeFileSync(contractsFile, generatedContracts(contractsFile, tree.contracts, tree.auth) +
            'type AssertJob<T extends import("@boringapi/core").JobDeclaration> = T;\n' +
            'type AssertJobHandler<F extends (ctx: C) => void | Promise<void>, C> = F;\n' +
            tree.jobs.map((job, index) => {
                const declaration = `typeof import(${JSON.stringify(moduleSpecifier(contractsFile, job.file))})`;
                const context = join(generatedRoot, relative(root, dirname(job.file)), "$types.d.ts");
                return `type Job${index} = AssertJob<${declaration}>;\ntype JobHandler${index} = AssertJobHandler<${declaration}["handler"], import(${JSON.stringify(moduleSpecifier(contractsFile, context).replace(/\.d$/, ""))}).JobContext>;`;
            }).join("\n") + "\n");
